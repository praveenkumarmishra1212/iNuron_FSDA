...
SELECT DISTINCT COUNTRY,CUSTOMER_KEY,CARD_NUMBER,'96524602' AS PIR_NBR,
     CASE 
     WHEN ACTIVITY_CODE LIKE 'S%' THEN 'TEST_CUST'
...
- SubChannel
- People Who Clicked
- People Who Opened  Campaign
- People Who Opted Out

Please include information for the next 30-days after launch (April 29th 2022)
*/
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
DROP TABLE AJ_CAMPAIGN_MEASUREMENT_TXN_US
CREATE TABLE AJ_CAMPAIGN_MEASUREMENT_TXN_US AS
SELECT RECORD_DATETIME,TXN_ID,SNDCUSTOMER_KEY,
        ...
        -- nvl(OTAL_CHARGES_USD,0) + NVL(NET_FOREIGN_EXCHANGE,0) AS REVENUE
       ...
       CASE WHEN (RECORD_DATETIME >= '2022-01-01' AND RECORD_DATETIME < '2023-01-01') THEN 'R12 PERIOD'
...
from  "WUDNA"."SUMMARY_GEN"."TXN_MASTER_SUPPLEMENT_V" as txn
LEFT OUTER JOIN "WUDNA"."SUMMARY_GEN"."CUSTOMER_MASTER_VW" as cmv
inner "WUDNA"."SUMMARY_GEN"."CUSTOMER_MASTER_VW" cmv on txn.CUST_ID = cmv.CUST_ID
inner join "WUDNA"."SUMMARY_GEN"."COUNTRY_VW" cv1 on txn.sndcountry_code = cv1.country_code
inner join "WUDNA"."SUMMARY_GEN"."COUNTRY_VW" cv2 on txn.rcvcountry_code = cv2.country_code
inner join "WUDNA"."SUMMARY_GEN"."AGENT_SUMMARY_VW" ag on txn.SNDAGENT_NUMBER = ag.AGENT_NUMBER
...
AND txn.RECORD_DATETIME >= '2022-04-29'AND txn.RECORD_DATETIME < '2021-05-31' 
AND SNDPRINCIPAL_USD > 0;
AND SNDCOUNTRY_CODE = 'US'
--IN ('IN','US','AU')
AND CUSTOMER_ID IN (SELECT DISTINCT CUSTOMER_KEY FROM AJ_CAR_128_MYWU_CUST_POINTS_REDEMPTIONS_BUCKET)
--('85754842','85756522','85756402','85756642')
...
grant select on YN_cust_level_flags_counts to anil_3456;
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
FROM
  (
    SELECT
      A.sndcustomer_key,
      A.CHANNEL_TYPE,
      ROW_NUMBER() OVER (PARTITION BY A.sndcustomer_key ORDER BY COUNT(TXN_ID) DESC,MAX(RECORD_DATETIME) DESC) TOP,
    count(1)
    FROM
...
--uncorrelated
select age,gender,income,emp_id from table a where emp_id not in (select distinct custom_id from from table b)
--------------------------------------------------------------------------------------------------------------------------
-- CHANNEL/SUB-CHANNEL/CUST_LEVEL_FLAGS
DROP TABLE AJ_CAMPAIGN_MEASUREMENT_CHANNEL_PAYIN;
CREATE OR REPLACE TABLE AJ_CAMPAIGN_MEASUREMENT_CHANNEL_PAYIN AS
...
( select 
 ...
 sndcustomer_key,
   ...
   count(case when TXN_PERIOD = 'R25-R36/PPR12 PERIOD'  then 1 end) as PPR12_txns,  
    ...
    COUNT(CASE WHEN PAY_IN_METHOD = 'CASH' THEN 1 END) AS  CASH_PAY_IN_COUNT,
...
--3RD HIGHEST SALARY IN EVERY DEPARTMENT
select *,ROW_NUMBER() OVER(PARTITION BY DEPT_NAME ORDER BY MAX(DESC) AS TOP_SALARY
FROM TABLE EMPLOYEE
WHERE TOP_SALARY <= 3                           
...
create table AJ_CAMPAIGN_MEASUREMENT_TXN_BUCKET as 
    ...
    LEFT JOIN
    (SELECT CUSTOMER_KEY,COUNT(TXN_ID) AS TXN_COUNT,
            CASE 
                 WHEN UNIT_PRICE_USD >= 1 AND UNIT_PRICE_USD < 1001  THEN '1$-1000$'
                 WHEN UNIT_PRICE_USD >= 1001 AND UNIT_PRICE_USD < 2001  THEN '10001$-2000$'
     
                 WHEN TXN_COUNT>=2 AND TXN_COUNT<6 THEN '1001$-2000$'
...
------------------------------------------------------------------------------FINANCIAL METRICS----------------------------------------------------------------------------
SELECT txn_bucket,SNDCOUNTRY_CODE,RCVCOUNTRY_CODE, PRICING_CHANNEL,FUNDS_IN,TEST_GH_CH,DATE_TRUNC('YEAR',RECORD_DATETIME) AS TXN_MONTH,
COUNT(DISTINCT SNDCUSTOMER_KEY) AS TOTAL_CUSTOMERS,
COUNT(TXN_ID) AS TXN_COUNT, SUM(PRINCIPAL) AS TOTAL_PRINCIPAL,SUM(FEE) AS TOTAL_FEE, SUM(NET_FX) AS TOTAL_NET_FX,
