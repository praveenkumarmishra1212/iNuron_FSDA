create database stored_proc_mysql ;
USE stored_proc_mysql;
...
-- first stored procedure to select all the records
DELIMITER && 
...
DELIMITER &&
CREATE procedure AVG_BAL_JOBROLE_param(IN VAR varchar(30))
BEGIN
 select avg(balance) from bank_details where job = upper(VAR);
END &&

CALL AVG_BAL_JOBROLE_param("self-employed");
...
CALL AVG_BAL_JOBROLE1("self-employed");

CALL AVG_BAL_JOBROLE1("AdMIN."); -- CASE INSENSITIVE IN MYSQL
